...
#if defined( USE_RADIO_ANT_SWITCH )
Gpio_t AntSwitch;
#endif
...
static void SetInterrupt( Gpio_t *obj, IrqModes irqMode, IrqPriorities irqPriority, GpioIrqHandler *irqHandler )
{
    GpioSetInterrupt( obj, irqMode, irqPriority, irqHandler);
}

void SX1276IoIrqInit( DioIrqHandler **irqHandlers )
{
    SetInterrupt( &SX1276.DIO0, IRQ_RISING_EDGE, IRQ_HIGH_PRIORITY, irqHandlers[0] );
    SetInterrupt( &SX1276.DIO1, IRQ_RISING_EDGE, IRQ_HIGH_PRIORITY, irqHandlers[1] );
    SetInterrupt( &SX1276.DIO2, IRQ_RISING_EDGE, IRQ_HIGH_PRIORITY, irqHandlers[2] );
    SetInterrupt( &SX1276.DIO3, IRQ_RISING_EDGE, IRQ_HIGH_PRIORITY, irqHandlers[3] );
    SetInterrupt( &SX1276.DIO4, IRQ_RISING_EDGE, IRQ_HIGH_PRIORITY, irqHandlers[4] );
    SetInterrupt( &SX1276.DIO5, IRQ_RISING_EDGE, IRQ_HIGH_PRIORITY, irqHandlers[5] );
}
...
{
    GpioInit( &RadioTCXO, RADIO_TCXO_EN, PIN_OUTPUT, PIN_PUSH_PULL, PIN_NO_PULL, 1 );
}

static void TCXO_ON(void){
	GpioWrite(&RadioTCXO, 1);
}

static void TCXO_OFF(void){
	GpioWrite(&RadioTCXO, 0);
}
