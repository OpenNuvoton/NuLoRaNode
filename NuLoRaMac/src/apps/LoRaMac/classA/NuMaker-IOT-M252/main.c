...
/*! \file classA/NuMaker-IOT-M252/main.c */
...
/*!
 * Load factory configure
 */

static char s_chFactoryBuf[FACTORY_EEPROM_SIZE];
static cJSON *s_psConfigRoot;

static int GetActivationOTAAFromConfig(
	uint32_t *pu32ActivationOTAA
)
{
	cJSON *psItem;
	cJSON *psArrayItem;
	int i;
	*pu32ActivationOTAA = 0;
	
	if(s_psConfigRoot == NULL)
		return -1;
	
	psItem = cJSON_GetObjectItem(s_psConfigRoot, "ActivationOTAA");
	
	if(psItem == NULL)
		return -1;
	
	*pu32ActivationOTAA = strtoul(psItem->valuestring, NULL, 0);
	
	return 0;
}

static int GetDeviceAddrFromConfig(
	uint32_t *pu32DeviceAddr
)
{
	cJSON *psItem;
	cJSON *psArrayItem;
	...
	psItem = cJSON_GetObjectItem(s_psConfigRoot, "DeviceAddr");
...
static int GetDeviceEUIFromConfig(
	uint8_t *au8DeviceEUI
)
{
	cJSON *psItem;
	cJSON *psArrayItem;
	...
	psItem = cJSON_GetObjectItem(s_psConfigRoot, "DEVICE_EUI");
	...
	for(i = 0; i < 8; i++){
		psArrayItem = cJSON_GetArrayItem(psItem, i);
		au8DeviceEUI[i] = strtoul(psArrayItem->valuestring, NULL, 0);
	}
	
	return 0;
}

static int GetJoinEUIFromConfig(
	uint8_t *au8JoinEUI
)
{
	cJSON *psItem;
	...
	psItem = cJSON_GetObjectItem(s_psConfigRoot, "JOIN_EUI");
	...
	for(i = 0; i < 8; i++){
		psArrayItem = cJSON_GetArrayItem(psItem, i);
		au8JoinEUI[i] = strtoul(psArrayItem->valuestring, NULL, 0);
	}
	
	return 0;
}

static int GetPinCodeFromConfig(
	uint8_t *au8PinCode
)
{
	cJSON *psItem;
	...
	psItem = cJSON_GetObjectItem(s_psConfigRoot, "PIN_CODE");
	...
	for(i = 0; i < 4; i++){
		psArrayItem = cJSON_GetArrayItem(psItem, i);
		au8PinCode[i] = strtoul(psArrayItem->valuestring, NULL, 0);
	}
	
	return 0;
}

static int GetAPPKeyFromConfig(
	uint8_t *au8APPKey
)
{
	cJSON *psItem;
	...
	psItem = cJSON_GetObjectItem(s_psConfigRoot, "APP_KEY");
	...
	for(i = 0; i < 16; i++){
		psArrayItem = cJSON_GetArrayItem(psItem, i);
		au8APPKey[i] = strtoul(psArrayItem->valuestring, NULL, 0);
	}
	
	return 0;
}

static int GetNWKKeyFromConfig(
	uint8_t *au8NWKKey
)
{
	cJSON *psItem;
	...
	psItem = cJSON_GetObjectItem(s_psConfigRoot, "NWK_KEY");
	...
	for(i = 0; i < 16; i++){
		psArrayItem = cJSON_GetArrayItem(psItem, i);
		au8NWKKey[i] = strtoul(psArrayItem->valuestring, NULL, 0);
	}
	
	return 0;
}

static int32_t LoadFactoryConfig(void)
{	
	cJSON *psItem;

	EepromReadBuffer(FACTORY_EEPROM_BASE - DATA_EEPROM_BASE, (uint8_t *)s_chFactoryBuf, FACTORY_EEPROM_SIZE);
	s_psConfigRoot = cJSON_Parse(s_chFactoryBuf);

	if(s_psConfigRoot == NULL){
		EepromWriteBuffer(FACTORY_EEPROM_BASE - DATA_EEPROM_BASE, (uint8_t *)g_factory_json, strlen(g_factory_json));
		EepromReadBuffer(FACTORY_EEPROM_BASE - DATA_EEPROM_BASE, (uint8_t *)s_chFactoryBuf, FACTORY_EEPROM_SIZE);
		s_psConfigRoot = cJSON_Parse(s_chFactoryBuf);
	}
...
//			AppDataSize = sprintf((char *)AppDataBuffer, "%d%d", u32Cnt, u32Cnt);
			AppDataSize = sprintf((char *)AppDataBuffer, "%d", u32Cnt);
			u32Cnt ++;
        }
        break;
    case 224:
...
int main( void )
{
    ...
    uint8_t appKey[16] = { 0 };   // Automatically filed from secure-element
    uint8_t nwkKey[16] = { 0 };   // Automatically filed from secure-element

    BoardInitMcu( );
    BoardInitPeriph( );

	if(LoadFactoryConfig() != 0)
	{
        printf( "LoRaMac wasn't properly initialized, error: unable load factory config\n" );
	...
	//Write secure-element DEV_EUI, JOI_EUI and SE_PIN, APP_KEY, NWK_KEY values.
	GetActivationOTAAFromConfig(&u32ActivationOTAA);
	GetDeviceAddrFromConfig(&DevAddr);

	GetDeviceEUIFromConfig(devEui);
	mibReq.Type = MIB_DEV_EUI;
	mibReq.Param.DevEui = devEui;
	LoRaMacMibSetRequestConfirm( &mibReq );
	
	GetJoinEUIFromConfig(joinEui);
	mibReq.Type = MIB_JOIN_EUI;
	mibReq.Param.JoinEui = joinEui;
	LoRaMacMibSetRequestConfirm( &mibReq );
	
	GetPinCodeFromConfig(sePin);
	...
	while( 1 )
    {
        ...
        switch( DeviceState )
        {
            ...
            case DEVICE_STATE_START:
            {
				...
				//Set channel mask. For TX: 904.7MHz,904.9MHz,905.1MHz,905.3MHz
				uint16_t channelsMask[6];
										  // Default channels mask value
				channelsMask[0] = 0xF000; // 0xFFFF;
				channelsMask[1] = 0x0000; // 0xFFFF;
				channelsMask[2] = 0x0000; // 0xFFFF;
				channelsMask[3] = 0x0000; // 0xFFFF;
				channelsMask[4] = 0x0000; // 0x00FF;
				channelsMask[5] = 0x0000; // 0x0000;
